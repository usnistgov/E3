...
public class Stream<A> {
    ...
    private static <A> Stream<A> merge(final Stream<A> ea, final Stream<A> eb) {
        ...
        Listener l1 = ea.listen_(left, out::send);
        Listener l2 = eb.listen_(right, out::send);
    ...
    public static <A> Stream<A> orElse(Iterable<Stream<A>> ss) {
        return Stream.merge(ss, (left, right) -> left);
    }
     ...
     * values, removing the {@link Optional} wrapper, discarding empty values.
     */
    ...
    public final Stream<A> gate(Cell<Boolean> c) {
        return Stream.filterOptional(
                snapshot(c, new Lambda2<A, Boolean, Optional<A>>() {
                    public Optional<A> apply(A a, Boolean pred) {
                        return pred ? Optional.of(a) : Optional.<A>empty();
                    }
                })
        );
    }
