...
public class Node<K, V> implements Tree<K, V> {
    ...
    public V get(K[] path) {
        ...
        throw new IllegalArgumentException("Path " + Arrays.toString(path) + " does not exist in tree");
    }
    ...
    public void combine(Tree<K, V> other) {
        if (other instanceof Leaf<K, V>)
            throw new IllegalStateException("Cannot add a leave to an already existing node");

        if (other instanceof Node<K, V> node) {
            for (var entry : node.branches.entrySet()) {
                if (this.branches.containsKey(entry.getKey())) {
                    this.branches.get(entry.getKey()).combine(entry.getValue());
                } else {
                    this.addTree(entry.getKey(), entry.getValue());
                }
            }
        }
    }
